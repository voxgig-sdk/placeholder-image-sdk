...
import (
	...
	omni "github.com/voxgig-sdk/placeholder-image-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PlaceholderImageError); is {
				out := map[string]any{
					"name":    "PlaceholderImageError",
...
func omniCtx(arg any, client *sdk.PlaceholderImageSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PlaceholderImageSDK); is {
			client = live
