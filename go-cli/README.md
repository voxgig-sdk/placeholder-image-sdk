...
boru-driven command-line client **and** interactive REPL for the PlaceholderImage
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/placeholder-image-cli)
make build

# 2. See usage (words, entities, env vars)
./placeholder-image-cli --help

# 3. Provide credentials once, via the environment
export PLACEHOLDER_IMAGE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./placeholder-image-cli load 1 placeholder            # {id:1} shorthand
./placeholder-image-cli load '{id:1}' placeholder       # explicit match map

# 5. Override the API base URL for a single call
PLACEHOLDER_IMAGE_BASE=https://api.example.com ./placeholder-image-cli load 1 placeholder

# 6. No arguments -> interactive REPL
./placeholder-image-cli
placeholder-image> load 1 placeholder
placeholder-image> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/placeholder-image-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/placeholder-image-cli load 1 placeholder
   ```
...
```sh
./placeholder-image-cli load 1 placeholder          # scalar shorthand for {id:1}
./placeholder-image-cli load '{id:1}' placeholder     # explicit match map
```
...
./placeholder-image-cli load 1 placeholder
```
...
Run with no arguments to open a REPL (prompt `placeholder-image>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `placeholder`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/placeholder-image-cli`. |
...
(`./placeholder-image-cli <expr>`) and interactively (the REPL), and expressions compose the
