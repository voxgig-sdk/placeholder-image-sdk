# PlaceholderImage SDK test runner
...
class PlaceholderImageTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PlaceholderImageTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PlaceholderImageTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PlaceholderImageTestRunner.getenv("PLACEHOLDER_IMAGE_TEST_LIVE")
        override = PlaceholderImageTestRunner.getenv("PLACEHOLDER_IMAGE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PlaceholderImageTestRunner.getenv(key)
        ...
        explain = PlaceholderImageTestRunner.getenv("PLACEHOLDER_IMAGE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PLACEHOLDER_IMAGE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PlaceholderImageTestRunner._test_control is not None:
            return PlaceholderImageTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PlaceholderImageTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PlaceholderImageTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PlaceholderImageTestRunner.load_test_control()
...
def load_env_local():
    PlaceholderImageTestRunner.load_env_local()


def env_override(m):
    return PlaceholderImageTestRunner.env_override(m)


def entity_data(v):
    return PlaceholderImageTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PlaceholderImageTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PlaceholderImageTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PlaceholderImageTestRunner.load_test_control()


def live_client_options():
    return PlaceholderImageTestRunner.live_client_options()


def live_delay_ms():
    return PlaceholderImageTestRunner.live_delay_ms()
