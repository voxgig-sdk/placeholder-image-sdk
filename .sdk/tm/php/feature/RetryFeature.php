...
// PlaceholderImage SDK retry feature
...
class PlaceholderImageRetryFeature extends PlaceholderImageBaseFeature
{
    ...
    public function init(PlaceholderImageContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PlaceholderImageContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PlaceholderImageContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PlaceholderImageContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
