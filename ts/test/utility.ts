...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.PLACEHOLDERIMAGE_TEST_LIVE ||
    'TRUE' === process.env.PLACEHOLDERIMAGE_TEST_OVERRIDE
  ) {
  ...
  m.PLACEHOLDERIMAGE_TEST_EXPLAIN = process.env.PLACEHOLDERIMAGE_TEST_EXPLAIN || m.PLACEHOLDERIMAGE_TEST_EXPLAIN
