...
//     (\`new PlaceholderImageSDK(...)\` / \`PlaceholderImageSDK.test(...)\` become
//     \`PlaceholderImageSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PlaceholderImageSDK'
...
const TEST_SEED = {"entity":{"placeholder":{"test01":{"id":"test01"}},"placeholder_image":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PlaceholderImageSDK(...)\` and any
// \`PlaceholderImageSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PlaceholderImageSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PlaceholderImageSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
