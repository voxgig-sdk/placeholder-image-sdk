...
describe('PlaceholderDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new PlaceholderImageSDK({
  ...
  test('direct-load-placeholder', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-placeholder', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PLACEHOLDER_IMAGE_TEST_PLACEHOLDER_ENTID']
