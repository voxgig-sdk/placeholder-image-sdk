...
(0, node_test_1.describe)('PlaceholderImageEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when PLACEHOLDER_IMAGE_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('PLACEHOLDER_IMAGE_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.PlaceholderImageSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'placeholder_image.' + op, live))
                return;
        }
        ...
        let placeholder_image_ref01_data = Object.values(setup.data.existing.placeholder_image)[0];
        // LOAD
        const placeholder_image_ref01_ent = client.PlaceholderImage();
        const placeholder_image_ref01_match_dt0 = {};
        const placeholder_image_ref01_data_dt0 = (await placeholder_image_ref01_ent.load(placeholder_image_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != placeholder_image_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/placeholder_image/PlaceholderImageTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.PlaceholderImageSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['placeholder_image01', 'placeholder_image02', 'placeholder_image03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.PlaceholderImageSDK(merge([
...
//# sourceMappingURL=PlaceholderImageEntity.test.js.map