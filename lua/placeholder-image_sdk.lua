...
require("placeholder-image_types")
...
local PlaceholderImageSDK = {}
PlaceholderImageSDK.__index = PlaceholderImageSDK
...
PlaceholderImageSDK._make_feature = _make_feature


function PlaceholderImageSDK.new(options)
  local self = setmetatable({}, PlaceholderImageSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PlaceholderImageSDK:options_map()
...
function PlaceholderImageSDK:get_utility()
  return Utility.copy(self._utility)
end


function PlaceholderImageSDK:get_root_ctx()
  return self._rootctx
end


function PlaceholderImageSDK:prepare(fetchargs)
...
function PlaceholderImageSDK:direct(fetchargs)
...
function PlaceholderImageSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function PlaceholderImageSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "PlaceholderImageSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function PlaceholderImageSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PlaceholderImageSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PlaceholderImageSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:PlaceholderImage():list() / client:PlaceholderImage():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function PlaceholderImageSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PlaceholderImageSDK.new(sdkopts)
  sdk.mode = "test"
