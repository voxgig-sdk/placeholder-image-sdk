...
local function make_error_util(ctx, err)
  ...
  local msg = "PlaceholderImageSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = PlaceholderImageError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == PlaceholderImageError then
