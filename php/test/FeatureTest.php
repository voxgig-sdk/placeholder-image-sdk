...
// PlaceholderImage SDK feature test
//
...
class FtCtrl extends PlaceholderImageControl
{
...
class FtClient
{
    ...
    private PlaceholderImageUtility $utility;

    public function __construct(PlaceholderImageUtility $utility, array $options)
    {
    ...
    public function get_utility(): PlaceholderImageUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PlaceholderImageContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PlaceholderImageContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PlaceholderImageContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PlaceholderImageConfig::make_feature($name);
    ...
    private static function buildUrl(PlaceholderImageSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PlaceholderImageError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PlaceholderImageResponse($fetched);
        }
    ...
    private function fail(PlaceholderImageContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PlaceholderImageContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
