...
// PlaceholderImage SDK PlaceholderImage entity
...
class PlaceholderImageEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PlaceholderImageEntity($this->_client, $opts);
    }

    /**
     * @param PlaceholderImage|array $args PlaceholderImage data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PlaceholderImageHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PlaceholderImage|array The current PlaceholderImage data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PlaceholderImage fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PlaceholderImageHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PlaceholderImage fields).
     */
    ...
    /**
     * Load a single PlaceholderImage.
     *
     * @param PlaceholderImageLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PlaceholderImageLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PlaceholderImage|array The loaded PlaceholderImage as an assoc-array at the
     *   SDK boundary; throws PlaceholderImageError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PlaceholderImageHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
