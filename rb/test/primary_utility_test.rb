...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = PlaceholderImageOmni.make_runner(TEST_JSON_FILE, PlaceholderImageSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    PlaceholderImageError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : PlaceholderImageOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = PlaceholderImageOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
