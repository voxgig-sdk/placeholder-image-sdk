...
class PlaceholderImageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = placeholder_image_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "placeholder_image." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set PLACEHOLDER_IMAGE_TEST_PLACEHOLDER_IMAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    placeholder_image_ref01_data = nil
    if placeholder_image_ref01_data_raw.length > 0
...
def placeholder_image_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "placeholder_image", "PlaceholderImageTestData.json")
  ...
  entid_env_raw = ENV["PLACEHOLDER_IMAGE_TEST_PLACEHOLDER_IMAGE_ENTID"]
