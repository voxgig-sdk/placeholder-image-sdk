...
module PlaceholderImageUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PlaceholderImageResponse.new({})
    ...
    if fetch_err
      ...
      response = PlaceholderImageResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PlaceholderImageResponse.new(fetched)
    else
