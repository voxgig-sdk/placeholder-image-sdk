...
[MCP](https://modelcontextprotocol.io) server exposing the PlaceholderImage SDK as
two agent tools — `placeholder-image_list` and `placeholder-image_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/placeholder-image-mcp)
make build

# 2. Provide credentials via the environment
export PLACEHOLDER_IMAGE_APIKEY=sk_live_xxx
...
```jsonc
// placeholder-image_list: first page of records
{ "entity": "placeholder" }
{ "entity": "placeholder", "query": { } }

// placeholder-image_load: one record by id
{ "entity": "placeholder", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/placeholder-image-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user placeholder-image \
     -- "$PWD"/dist/*/placeholder-image-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `placeholder-image_list` and `placeholder-image_load` tools now appear
   in new sessions. Ask the agent to *"list placeholder using placeholder-image"*
   and it calls `placeholder-image_list` with `{"entity":"placeholder"}`.
...
```sh
export PLACEHOLDER_IMAGE_APIKEY=sk_live_xxx            # API key
export PLACEHOLDER_IMAGE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `placeholder-image_list` tool
...
### Call the `placeholder-image_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `placeholder-image_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `placeholder-image_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PLACEHOLDER_IMAGE_APIKEY` | API key sent with every request. |
| `PLACEHOLDER_IMAGE_BASE` | Optional override of the API base URL. |
